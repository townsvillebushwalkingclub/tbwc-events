 ...
 * 2. Add FACEBOOK_APP_ID, FACEBOOK_APP_SECRET, and FACEBOOK_ACCESS_TOKEN to .env.local
 * 3. Run: npx tsx tools/get-long-lived-token.ts
...
function loadEnvFile(): { envVars: Record<string, string>; envFile: string | null } {
...
function makeRequest(url: string): Promise<Record<string, unknown>> {
...
async function getPageAccessToken(userAccessToken: string): Promise<string | null> {
  try {
    ...
    const data = (await makeRequest(url)) as { data?: Array<{ id: string; name: string; access_token: string }> }
    if (data.data && data.data.length > 0) {
      let pageData: { id: string; name: string; access_token: string } | null = null
      if (FACEBOOK_PAGE_ID) {
        pageData = data.data.find((p) => p.id === FACEBOOK_PAGE_ID) ?? null
        if (!pageData) {
          console.log(`⚠️  Page with ID ${FACEBOOK_PAGE_ID} not found`)
          console.log('📋 Available pages:')
          data.data.forEach((page) => console.log(`   - ${page.name} (ID: ${page.id})`))
      ...
      if (pageData) {
        ...
        console.log('🎉 This token NEVER EXPIRES (unless permissions are revoked)!')
    ...
    console.log('💡 Make sure you have admin access to the Facebook page')
    return null
  } catch (error) {
    console.error('❌ Error getting page access token:', (error as Error).message)
    return null
  }
}

async function getLongLivedToken(): Promise<void> {
  try {
    ...
    const data = (await makeRequest(url)) as { access_token?: string; expires_in?: number }
    if (data.access_token) {
      console.log('✅ Long-lived user token obtained successfully!')
      console.log('⏰ User token expires in:', data.expires_in, 'seconds (', Math.round((data.expires_in ?? 0) / 86400), 'days)')
...
if (!FACEBOOK_APP_ID || !FACEBOOK_APP_SECRET || !SHORT_LIVED_TOKEN) {
  ...
  console.log('2. Short-lived token: https://developers.facebook.com/tools/explorer/')
  ...
  console.log('3. Page ID: Check your Facebook Page settings or About section')
} else {
